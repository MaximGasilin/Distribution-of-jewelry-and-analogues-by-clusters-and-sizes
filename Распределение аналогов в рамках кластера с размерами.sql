...
BEGIN /* Заполнение вспомогательных данных. */
		...
		INSERT INTO #БатчиРазмеров(Батч, Размер)
		SELECT
			'Батч 2' AS Батч
			,'15' AS Размер
		UNION ALL
		...
		SELECT
			'Батч 2' AS Батч
			,'22' AS Размер
		CREATE INDEX IDX ON #БатчиРазмеров(Батч, Размер)
	...
	--SELECT * FROM #МаксимальныеОстаткиТТ
		...
		BEGIN /*Пятая фаза. 
				...
				BEGIN /*Шаг. Присвоение очередных рейтингов в случае большого выбора. */			
					...
					*/	
					...
					WHERE
						ОТ.РейтингКНДП IS NULL
						AND ОТ.СтопЛист = 0x00
				/*Шаг. Присвоение очередных рейтингов в случае большого выбора. Окончание.*/
				END
			...
			BEGIN /* Распределение согласно новым рейтингам сортрировки.*/
				...
				DROP TABLE IF EXISTS #КоэффициентыБатчей
				CREATE TABLE #КоэффициентыБатчей(
					Кластер varchar(50)
					,Номенклатура varchar(50)
					,Батч varchar(50)
				...
				INSERT INTO #ПланируемоеОбщееКоличествоНаТТПослеРаспределения (Кластер, ТорговаяТочка, Количество)
				SELECT
					ОТ.Кластер AS Кластер
					,ОТ.ТорговаяТочка AS ТорговаяТочка
					,SUM(ОТ.Количество) AS Количество
				...
				CREATE INDEX IDX ON #ПланируемоеОбщееКоличествоНаТТПослеРаспределения (Кластер, ТорговаяТочка) 

				INSERT INTO #КоэффициентыБатчей(Кластер, Номенклатура, Батч, Коэффициент)
				SELECT 
					ОТ.КластерДляПодсортировки AS Кластер 
					,ОТ.Номенклатура AS Номенклатура
					,ВТ_1.Батч AS Батч
				...
				CREATE INDEX IDX ON #КоэффициентыБатчей (Кластер, Номенклатура, Батч) 
				
				SELECT '#КоэффициентыБатчей' AS '#КоэффициентыБатчей'
				SELECT * FROM #КоэффициентыБатчей

				SELECT @ОсталосьРаспределить = SUM(ОТ.Количество) FROM #ОпорныйПлан AS ОТ
				SET @ОстаВалосьРаспределить = 0

				WHILE @ОсталосьРаспределить > 0 AND (@ОстаВалосьРаспределить <> @ОсталосьРаспределить)
				BEGIN
					...
					SELECT 
						...
						--,В_ТН.Номер AS ПорядокВГруппировке
						,ОТ.Количество AS Количество
						,ВТ_1.Количество * (В_ПРБ.РаспределятьПолностью + (1 - В_ПРБ.РаспределятьПолностью) * В_КБ.Коэффициент) AS ПотребностьПоТТ -- Смысл формулы в следующем: Если распределять весь остаток, то умножаем на 1, а если нужно взять лишь часть потребности на ТТ, то умножаем на коэффициент.
						,ROW_NUMBER() OVER (PARTITION BY
												ОТ.Кластер 
												,ОТ.Номенклатура
												,ВТ_2.ХарактеристикаНоменклатуры
											ORDER BY 
												ВТ_1.Количество * В_ПРБ.ПорядокСортировкиТТ ASC
					...
					INNER JOIN #КоэффициентыБатчей AS В_КБ
						ON ОТ.Кластер = В_КБ.Кластер
						AND ОТ.Номенклатура = В_КБ.Номенклатура
						AND В_БР.Батч = В_КБ.Батч
					WHERE ОТ.Количество > 0
						AND CAST(ВТ_1.Количество * (В_ПРБ.РаспределятьПолностью + (1 - В_ПРБ.РаспределятьПолностью) * В_КБ.Коэффициент) AS numeric(10)) >= ВТ_2.РейтингКНДП
