...
BEGIN /* Печать тестовых данных. */
	
	SELECT '4184f3a' AS 'Версия'
	...
	--SELECT * FROM #ОстаткиТТ_СХарактеристиками
	...
	--SELECT * FROM #МаксимальныеОстаткиТТ
		...
		BEGIN /*Пятая фаза. 
			...
			BEGIN /* Расчет коэффициентов для каждого размера на каждой ТТ. */
				...
				FROM #ОстаткиСклада_СХарактеристиками AS ОТ WITH(NOLOCK)
				GROUP BY
					ОТ.КластерДляПодсортировки
					,ОТ.Номенклатура
				...
				INSERT INTO #КоэффициентыРазмеровДляНоменклатуры(Кластер, Номенклатура,
				...
				SELECT 
					...
					,SUM(ОТ.Количество) / ВТ.КоличествоРазмеров AS Коэффициент
				FROM #ОпорныйПлан AS ОТ WITH(NOLOCK)
				LEFT JOIN #КоличествоРазмеровДляНоменклатуры AS ВТ WITH(NOLOCK)
					ON ОТ.Кластер = ВТ.Кластер
					AND ОТ.Номенклатура = ВТ.Номенклатура
				GROUP BY
					ОТ.Кластер
					,ОТ.Номенклатура
					,ВТ.КоличествоРазмеров
				...
				FROM #ОстаткиСклада_СХарактеристиками AS ОТ WITH(NOLOCK)
				LEFT JOIN #КоличествоТорговыхТочекДляХарактеристики AS ВТ WITH(NOLOCK)
					ON ОТ.КластерДляПодсортировки = ВТ.Кластер
				GROUP BY
					ОТ.КластерДляПодсортировки
					,ОТ.Номенклатура
				...
				FROM #КоэффициентыТорговыхТочекДляХарактеристики AS ОТ WITH(NOLOCK)
				INNER JOIN #КоэффициентыРазмеровДляНоменклатуры AS ВТ WITH(NOLOCK)
				ON ОТ.Кластер = ВТ.Кластер
				AND ОТ.Номенклатура = ВТ.Номенклатура
			...
			END
				...
				SELECT
					...
					,SUM(CASE WHEN ОТ.ХарактеристикаНоменклатуры = '22'   THEN ОТ.Коэффициент ELSE 0 END) AS '22'
				FROM 
					#ТекущаяМартицаКоэффициентов AS ОТ WITH(NOLOCK)
				GROUP BY
					ОТ.Кластер
					,ОТ.ТорговаяТочка
				*/
			...
			BEGIN /* Сортировка размеров исходя из необходимости распределять хотя-бы через размер. */
				...
				FROM #ТекущаяМартицаКоэффициентов AS ОТ WITH(NOLOCK)
				INNER JOIN #БатчиРазмеров AS ВТ_1 WITH(NOLOCK)
					ON ОТ.ХарактеристикаНоменклатуры = ВТ_1.Размер
				INNER JOIN #ПорядокБатчейРазмеров AS ВТ_2 WITH(NOLOCK)
				...
				FROM 
					#ЧерновикРейтингаРазмеров AS ОТ WITH(NOLOCK)
				GROUP BY
					ОТ.Кластер
					,ОТ.Номенклатура
					,ОТ.Батч
				...
				*/
				--SELECT * FROM #ЧерновикРейтингаРазмеров WITH(NOLOCK)
				...
				FROM #ЧерновикРейтингаРазмеров AS ОТ WITH(NOLOCK)
				LEFT JOIN #РейтингКлассовСовместимостиРазмеров AS ВТ_1 WITH(NOLOCK)
					ON ОТ.Кластер = ВТ_1.Кластер
					AND ОТ.Номенклатура = ВТ_1.Номенклатура
				...
				SELECT * FROM #РейтингРазмеров WITH(NOLOCK)
				SELECT
					ОТ.Кластер AS Кластер
					,ОТ.Номенклатура AS Номенклатура
					,ОТ.Батч AS Батч
				...
				FROM 
					#РейтингРазмеров AS ОТ WITH(NOLOCK)
				GROUP BY
					ОТ.Кластер
					,ОТ.Номенклатура 
					,ОТ.Батч
				*/
				...
				--RETURN
				/*
				SELECT * FROM #РейтингРазмеров AS ОТ WITH(NOLOCK)
				WHERE
			...
			BEGIN /* Распределение согласно новым рейтингам сортрировки.*/
				...
				BEGIN /* Вспомогательные таблицы для этого этапа */
					...
					CREATE TABLE #ПланРаспределенияСХарактеристиками_Промежуточный(
																	...
																	,ТорговаяТочка varchar(50)
																	,Количество numeric(10)
																	,КоличествоКумулятивное numeric(10)
																	)
					CREATE INDEX IDX ON #ПланРаспределенияСХарактеристиками_Промежуточный(Кластер, ТорговаяТочка, Номенклатура, ХарактеристикаНоменклатуры)
					...
					DROP TABLE IF EXISTS #НеУчтенныеОстаткиХарактеристикНаТТ
					/* 
					В эту таблицу перед началом распределения будут скопированы остатки на ТТ по характеристикам.
					На каждом шаге алгоритма при получении очередной порции распределения из нее будут удаляться размеры,
					которые в данный момент уже есть на ТТ. И которые еще не удалялись ранее. Информация об удаляемых размерах
					и будет храниться в таблице #НеУчтенныеОстаткиХарактеристикНаТТ. После каждого шага таблица будет корректироваться.
					*/
					CREATE TABLE #НеУчтенныеОстаткиХарактеристикНаТТ(
						ТорговаяТочка varchar(50)
						,Кластер varchar(50)
						--,Номенклатура varchar(50)
						,Размер varchar(50)
					...
					--CREATE INDEX IDX ON #НеУчтенныеОстаткиХарактеристикНаТТ(ТорговаяТочка, Кластер, Номенклатура, Размер)
					CREATE INDEX IDX ON #НеУчтенныеОстаткиХарактеристикНаТТ(ТорговаяТочка, Кластер, Размер)

					DROP TABLE IF EXISTS #ПересечениеОстатковХарактеристикНаТТИТекущегоРаспределения
					/* 
					Используется для получения пересечения между распределенными и еще не учтенными из остатков на ТТ.
					*/
					CREATE TABLE #ПересечениеОстатковХарактеристикНаТТИТекущегоРаспределения(
						ТорговаяТочка varchar(50)
						,Кластер varchar(50)
						--,Номенклатура varchar(50)
						,Размер varchar(50)
						,КоличествоРаспределено Numeric(10)
						,КоличествоНаТТ Numeric(10)
						,Количество Numeric(10)
						)
					--CREATE INDEX IDX ON #ПересечениеОстатковХарактеристикНаТТИТекущегоРаспределения(ТорговаяТочка, Кластер, Номенклатура, Размер)
					CREATE INDEX IDX ON #ПересечениеОстатковХарактеристикНаТТИТекущегоРаспределения(ТорговаяТочка, Кластер, Размер)

				/* Вспомогательные таблицы для этого этапа */
				END
				
				INSERT INTO	#НеУчтенныеОстаткиХарактеристикНаТТ(ТорговаяТочка, Кластер, Размер, Количество)
				SELECT
					ОТ.ТорговаяТочка AS ТорговаяТочка
					,ОТ.КластерДляПодсортировки AS Кластер
					,ОТ.ХарактеристикаНоменклатуры AS Размер
					,SUM(ОТ.Количество) AS Количество
				FROM #ОстаткиТТ_СХарактеристиками AS ОТ WITH(NOLOCK)
				WHERE ОТ.Количество > 0
				GROUP BY
					ОТ.ТорговаяТочка
					,ОТ.КластерДляПодсортировки
				...
				SELECT * 
				FROM #ПараметрыБатчей AS ОТ WITH(NOLOCK)
				LEFT JOIN #ПорядокБатчейРазмеров AS ВТ WITH(NOLOCK)
					ON ОТ.Батч = ВТ.Батч
				ORDER BY 
					ВТ.Порядок
				...
				LEFT JOIN #ПараметрыБатчей AS ВТ WITH(NOLOCK)
					ON ОТ.Кластер = ВТ.Кластер
					AND ОТ.Номенклатура = ВТ.Номенклатура
				GROUP BY
					ОТ.Кластер
				...
				BEGIN
					...
					FROM #Предложение_III AS ОТ WITH(NOLOCK)
					WHERE
					...
					INNER JOIN #ПорядокБатчейРазмеров AS ВТ_2 WITH(NOLOCK)
						ON ВТ_1.Батч = ВТ_2.Батч 
					INNER JOIN #ПараметрыБатчей AS ВТ_3 WITH(NOLOCK)
					...
					SELECT * FROM #ОстаткиБатчей AS ОТ WITH(NOLOCK)
					WHERE ОТ.Маркер = 1
					SELECT * FROM #ОстаткиБатчей AS ОТ WITH(NOLOCK)
					...
					FROM #РейтингРазмеров AS ОТ WITH(NOLOCK)
					INNER JOIN (SELECT
									ОТ.Кластер AS Кластер
									,ОТ.Номенклатура AS Номенклатура
									,ОТ.ХарактеристикаНоменклатуры AS ХарактеристикаНоменклатуры
					...
					INNER JOIN #БатчиРазмеров AS ВТ_1 WITH(NOLOCK)
						ON ОТ.ХарактеристикаНоменклатуры = ВТ_1.Размер
					INNER JOIN #ОстаткиБатчей AS ВТ_2 WITH(NOLOCK)
						ON ОТ.Кластер = ВТ_2.Кластер
						AND ОТ.Номенклатура = ВТ_2.Номенклатура
						AND ОТ.Номенклатура = ВТ_2.Номенклатура
					...
					SELECT
						...
						,SUM(CASE WHEN ОТ.ХарактеристикаНоменклатуры = '22'   THEN ОТ.РейтингКНДП ELSE 0 END) AS '22'
					FROM 
						#ТекущиеРейтингиРазмеров AS ОТ WITH(NOLOCK)
					GROUP BY
						ОТ.Кластер
						,ОТ.Номенклатура
					
					SELECT * FROM #Спрос_III WITH(NOLOCK)
					...
					FROM #ОпорныйПлан AS ОТ WITH(NOLOCK)
					LEFT JOIN #ТекущиеРейтингиРазмеров AS ВТ_2 WITH(NOLOCK)
						ON ОТ.Кластер = ВТ_2.Кластер
						--AND ОТ.Характ = ВТ_2.ХарактеристикаНоменклатуры
						AND ОТ.Номенклатура = ВТ_2.Номенклатура
						--AND ВТ_1.Количество >= ВТ_2.РейтингКНДП
					INNER JOIN #БатчиРазмеров AS В_БР WITH(NOLOCK)
						ON ВТ_2.ХарактеристикаНоменклатуры = В_БР.Размер
					INNER JOIN #ПорядокБатчейРазмеров AS В_ПРБ WITH(NOLOCK)
						ON В_БР.Батч = В_ПРБ.Батч
					INNER JOIN #ПараметрыБатчей AS В_КБ WITH(NOLOCK)
						ON ОТ.Кластер = В_КБ.Кластер
						AND ОТ.Номенклатура = В_КБ.Номенклатура
						AND В_БР.Батч = В_КБ.Батч
					LEFT JOIN #ПланируемоеОбщееКоличествоНаТТПослеРаспределения AS ВТ_1 WITH(NOLOCK)
						ON ОТ.Кластер = ВТ_1.Кластер
						AND ОТ.ТорговаяТочка = ВТ_1.ТорговаяТочка
						AND ОТ.Номенклатура = ВТ_1.Номенклатура
					...
					SELECT 
						...
						,ОТ.ТорговаяТочка
						,ОТ.ЦифровойМаркерВнутриНоменклатуры
					FROM #Спрос_III AS ОТ WITH(NOLOCK)
					WHERE
						ОТ.ХарактеристикаНоменклатуры IN('16,5', '17,5')
					...
					/* Получение главной промежутчной таблицы. В ней находятся текущие данные, которые нужно добавить к распределению
					и отнять от остатков склада.*/

					TRUNCATE TABLE #ПланРаспределенияСХарактеристиками_Промежуточный
					INSERT INTO #ПланРаспределенияСХарактеристиками_Промежуточный (Кластер, Номенклатура, ХарактеристикаНоменклатуры, ТорговаяТочка, Количество, КоличествоКумулятивное)
					SELECT
						ОТ.Кластер AS Кластер
						,ОТ.Номенклатура AS Номенклатура
						...
						,ОТ.ТорговаяТочка AS ТорговаяТочка
						,SUM(1) AS Количество
						,SUM(1) OVER (PARTITION BY ОТ.Кластер, ОТ.ТорговаяТочка, ВТ.ХарактеристикаНоменклатуры ORDER BY ОТ.Номенклатура) AS КоличествоКумулятивное
					FROM #Спрос_III AS ОТ WITH(NOLOCK)
					INNER JOIN #Предложение_III AS ВТ WITH(NOLOCK)
						ON ОТ.Кластер = ВТ.Кластер
						AND ОТ.Номенклатура = ВТ.Номенклатура
						AND ОТ.ХарактеристикаНоменклатуры = ВТ.ХарактеристикаНоменклатуры
					...
					--SELECT '#ПланРаспределенияСХарактеристиками_Промежуточный' AS '#ПланРаспределенияСХарактеристиками_Промежуточный'
					--SELECT *
					--FROM #ПланРаспределенияСХарактеристиками_Промежуточный AS ОТ WITH(NOLOCK)

					/*--------------------------------------------------------------------------------------------------------*/

					TRUNCATE TABLE #ПересечениеОстатковХарактеристикНаТТИТекущегоРаспределения
					INSERT INTO	#ПересечениеОстатковХарактеристикНаТТИТекущегоРаспределения(ТорговаяТочка, Кластер, Размер, КоличествоРаспределено, КоличествоНаТТ, Количество)
					SELECT
						ОТ.ТорговаяТочка AS ТорговаяТочка
						,ОТ.Кластер AS Кластер
						,ОТ.ХарактеристикаНоменклатуры AS Размер
						,SUM(ОТ.Количество) AS КоличествоРаспределено
						,SUM(ВТ.Количество) AS КоличествоНаТТ
						,(SUM(ОТ.Количество) + SUM(ВТ.Количество) - ABS(SUM(ОТ.Количество) - SUM(ВТ.Количество))) / 2 AS Количество
					FROM #ПланРаспределенияСХарактеристиками_Промежуточный AS ОТ WITH(NOLOCK)
					INNER JOIN #НеУчтенныеОстаткиХарактеристикНаТТ AS ВТ WITH(NOLOCK)
						ON ОТ.ТорговаяТочка = ВТ.ТорговаяТочка
						AND ОТ.Кластер = ВТ.Кластер
						AND ОТ.ХарактеристикаНоменклатуры = ВТ.Размер
					WHERE ВТ.Количество > 0
					GROUP BY 
						ОТ.ТорговаяТочка
						,ОТ.Кластер
						,ОТ.ХарактеристикаНоменклатуры
					...
					IF @@ROWCOUNT > 0 
					BEGIN
						SELECT '#ПересечениеОстатковХарактеристикНаТТИТекущегоРаспределения' AS '#ПересечениеОстатковХарактеристикНаТТИТекущегоРаспределения'
						SELECT *
						FROM #ПересечениеОстатковХарактеристикНаТТИТекущегоРаспределения

						SELECT '#ПланРаспределенияСХарактеристиками_Промежуточный' AS '#ПланРаспределенияСХарактеристиками_Промежуточный'
						SELECT 
							ОТ.Кластер AS Кластер_ОТ
							,ОТ.ТорговаяТочка AS ТорговаяТочка_ОТ
							,ОТ.Номенклатура AS Номенклатура_ОТ
							,ОТ.ХарактеристикаНоменклатуры AS ХарактеристикаНоменклатуры_ОТ
							,ОТ.Количество AS Количество_ОТ
							--,SUM(ОТ.Количество) OVER (PARTITION BY ОТ.Кластер, ОТ.ХарактеристикаНоменклатуры ORDER BY ОТ.ТорговаяТочка) AS КумулятивноеКоличество_ОТ
							,ОТ.КоличествоКумулятивное AS КоличествоКумулятивное_ОТ
							,ВТ.Кластер AS Кластер_ВТ
							,ВТ.ТорговаяТочка AS ТорговаяТочка_ВТ
							,ВТ.Размер AS Размер_ВТ
							,ВТ.КоличествоРаспределено AS КоличествоРаспределено_ВТ
							,ВТ.КоличествоНаТТ AS КоличествоНаТТ_ВТ
							,ВТ.Количество AS Количество_ВТ
						FROM #ПланРаспределенияСХарактеристиками_Промежуточный AS ОТ WITH(NOLOCK)
						INNER JOIN #ПересечениеОстатковХарактеристикНаТТИТекущегоРаспределения AS ВТ WITH(NOLOCK)
							ON ОТ.Кластер = ВТ.Кластер
							--AND ОТ.Номенклатура = ВТ.Номенклатура
							AND ОТ.ХарактеристикаНоменклатуры = ВТ.Размер
							AND ОТ.ТорговаяТочка = ВТ.ТорговаяТочка
						WHERE
							ОТ.КоличествоКумулятивное <= ВТ.Количество	
						...
						WHERE
							ОТ.КоличествоКумулятивное <= ВТ.Количество	
						/*
						DELETE #ПланРаспределенияСХарактеристиками_Промежуточный
						FROM #ПланРаспределенияСХарактеристиками_Промежуточный AS ОТ WITH(NOLOCK)
						INNER JOIN #ПересечениеОстатковХарактеристикНаТТИТекущегоРаспределения AS ВТ WITH(NOLOCK)
							ON ОТ.ТорговаяТочка = ВТ.ТорговаяТочка
							AND ОТ.Кластер = ВТ.Кластер
						...
						*/
						...
						SELECT '#НеУчтенныеОстаткиХарактеристикНаТТ' AS '#НеУчтенныеОстаткиХарактеристикНаТТ'
						SELECT *
						FROM #НеУчтенныеОстаткиХарактеристикНаТТ AS ОТ WITH(NOLOCK)
						INNER JOIN #ПересечениеОстатковХарактеристикНаТТИТекущегоРаспределения AS ВТ WITH(NOLOCK)
							ON ОТ.Кластер = ВТ.Кластер
							--AND ОТ.Номенклатура = ВТ.Номенклатура
							AND ОТ.Размер = ВТ.Размер
							AND ОТ.ТорговаяТочка = ВТ.ТорговаяТочка
					...
					UPDATE ОТ
						SET ОТ.Количество = ОТ.Количество + ВТ.Количество
					FROM #ПланРаспределенияСХарактеристиками AS ОТ WITH(NOLOCK)
					INNER JOIN #ПланРаспределенияСХарактеристиками_Промежуточный AS ВТ WITH(NOLOCK)
						ON ОТ.Кластер = ВТ.Кластер
						AND ОТ.Номенклатура = ВТ.Номенклатура
						AND ОТ.ХарактеристикаНоменклатуры = ВТ.ХарактеристикаНоменклатуры
						AND ОТ.ТорговаяТочка = ВТ.ТорговаяТочка
					...
					SELECT
						...
						,ОТ.Количество AS Количество
					FROM #ПланРаспределенияСХарактеристиками_Промежуточный AS ОТ WITH(NOLOCK)
					LEFT JOIN #ПланРаспределенияСХарактеристиками AS ВТ WITH(NOLOCK)
						ON ОТ.Кластер = ВТ.Кластер
						AND ОТ.Номенклатура = ВТ.Номенклатура
						AND ОТ.ХарактеристикаНоменклатуры = ВТ.ХарактеристикаНоменклатуры
						AND ОТ.ТорговаяТочка = ВТ.ТорговаяТочка
					...
					FROM #ПланируемоеОбщееКоличествоНаТТПослеРаспределения AS ОТ WITH(NOLOCK)
					INNER JOIN (SELECT
									ОТ.Кластер AS Кластер
									,ОТ.Номенклатура AS Номенклатура
									,ОТ.ТорговаяТочка AS ТорговаяТочка					
					...
					INNER JOIN (SELECT
									...
									,SUM(ОТ.Количество) AS Количество
								FROM #ПланРаспределенияСХарактеристиками_Промежуточный AS ОТ WITH(NOLOCK)
								LEFT JOIN #БатчиРазмеров AS ВТ WITH(NOLOCK)
					...
					SELECT @ОсталосьРаспределить = SUM(ОТ.Количество) FROM #ОпорныйПлан AS ОТ WITH(NOLOCK)

					UPDATE ОТ
						SET ОТ.БатчОбработан = 0x01
					FROM #ПараметрыБатчей AS ОТ WITH(NOLOCK)
					LEFT JOIN #ОстаткиБатчей AS ВТ WITH(NOLOCK)
						ON ОТ.Батч = ВТ.Батч
					...
					BEGIN
						UPDATE ОТ
							SET ОТ.БатчОбработан = 0x01
						FROM #ПараметрыБатчей AS ОТ WITH(NOLOCK)
						INNER JOIN #ОстаткиБатчей AS ВТ WITH(NOLOCK)
							ON ОТ.Батч = ВТ.Батч
							AND ОТ.БатчОбработан = 0x00
							AND ВТ.Маркер = 1
					END

					SELECT @ОсталосьБатчей = COUNT(ОТ.Батч) 
					FROM #ПараметрыБатчей AS ОТ WITH(NOLOCK) 
					WHERE ОТ.БатчОбработан = 0x00
				...
				FROM 
					(SELECT
						...
						,ОТ.Количество AS Количество
					 FROM
						#ПланРаспределенияСХарактеристиками AS ОТ WITH(NOLOCK)
					...
					SELECT
						...
						,ОТ.Количество AS Количество
					 FROM
						#ОстаткиТТ_СХарактеристиками AS ОТ WITH(NOLOCK)
					) AS ОТ
				GROUP BY
					ОТ.Кластер
					,ОТ.ТорговаяТочка
